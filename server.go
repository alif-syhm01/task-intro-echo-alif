...
func CreateUserController(c echo.Context) error {
	...
	formAddress := c.FormValue("address")
	...
	// insert the rest of field
	user.Name = formName
	user.Email = formEmail
	user.Address = formAddress
...
func GetUsersController(c echo.Context) error {
	...
	return c.JSON(http.StatusOK, map[string]interface{}{
		"messages": fmt.Sprintf("success, %d found data", len(users)),
		"users":    users,
	})
}
...
func UpdateUserController(c echo.Context) error {
	...
	for i, v := range users {
		if v.Id == id {
			// re-assign with the new data
			input.Id = v.Id
			input.Name = formName
			input.Email = formEmail
			input.Address = formAddress
